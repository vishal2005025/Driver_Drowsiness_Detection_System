...
# Try to use pygame; fallback to ffplay
try:
    from pygame import mixer
    mixer.init()
    use_pygame = True
except Exception as e:
    print(f"[WARNING] Pygame failed: {e}")
    use_pygame = False

# Sound paths
NO_DRIVER_SOUND_PATH = 'static/nodriver_audio.wav'
SLEEP_SOUND_PATH = 'static/sleep_sound.wav'
TIRED_SOUND_PATH = 'static/rest_audio.wav'

if use_pygame:
    no_driver_sound = mixer.Sound(NO_DRIVER_SOUND_PATH)
    sleep_sound = mixer.Sound(SLEEP_SOUND_PATH)
    tired_sound = mixer.Sound(TIRED_SOUND_PATH)

def play_sound(sound_name):
    if use_pygame:
        if sound_name == "sleep":
            sleep_sound.play()
        elif sound_name == "no_driver":
            no_driver_sound.play()
        elif sound_name == "tired":
            tired_sound.play()
    else:
        sound_path = {
            "sleep": SLEEP_SOUND_PATH,
            "no_driver": NO_DRIVER_SOUND_PATH,
            "tired": TIRED_SOUND_PATH
        }.get(sound_name)
        if sound_path:
            subprocess.Popen(
...
def detech():
    ...
    while True:
        ...
        if faces:
            ...
            if time.time() - no_driver_time > 5:
                if no_driver_sound_flag == 0:
                    play_sound("no_driver")
                    no_driver_sound_start = time.time()
                elif time.time() - no_driver_sound_start > 3:
                    play_sound("no_driver")
                    no_driver_sound_start = time.time()
                no_driver_sound_flag = 1
